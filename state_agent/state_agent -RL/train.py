...
from state_agent.planner import Planner, network_features_v2
...
if __name__ == "__main__":
    ...
    n_epochs = 60
    n_trajectories = 1  # so, no. of timesteps in each epoch = MAX_FRAMES_TRAIN*10  (which is 100*10=1000 for now)
    batch_size = 40  # maybe keep this constant
    n_iterations = 30  # vary this to match: n_trajectories * MAX_FRAMES_TRAIN = batch_size * n_iterations
    ...
    player1_net = Planner(21, 64, 6).to(device)
    player2_net = Planner(21, 64, 6).to(device)
    ...
    action_optim1 = torch.optim.Adam(player1_net.parameters(), lr=0.0001 )
    action_optim2 = torch.optim.Adam(player2_net.parameters(), lr=0.0001)
    value_optim1 = torch.optim.Adam(value_net1.parameters(), lr=0.0001)
    value_optim2 = torch.optim.Adam(value_net2.parameters(), lr=0.0001)
    ...
    for epoch in range(n_epochs): #main PPO loop
            ...
            advantages1 = compute_gae(returns1, state_values1, 0.99, 0.95)
            advantages2 = compute_gae(returns2, state_values2, 0.99, 0.95)
            ...
            for it in range(n_iterations):
                ...
                for player_net, features, action_ids, returns, advantages, log_probs_old, value_net, value_optim, action_optim in [
                        (player1_net, features1, action_ids1, returns1, advantages1, log_probs_old1, value_net1,
                         value_optim1, action_optim1),
                        (player2_net, features2, action_ids2, returns2, advantages2, log_probs_old2, value_net2,
                         value_optim2, action_optim2)
                    ]:
