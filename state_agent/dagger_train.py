...
def train():
    ...
    train_features_imitation0 = torch.stack([torch.cat([network_features(d['player_state'][0], d['opponent_state'], d['soccer_state'], 0 if (i % MAX_FRAMES) < n_trajectories/2 else 1),
                                d['ball_velocity'], d['kart_velocity'][0], d['opp_velocity'][0], d['opp_velocity'][1]]) for i, d in enumerate(train_data_imitation)]).to(device).float()
    ...
    train_features_imitation1 = torch.stack([torch.cat([network_features(d['player_state'][1], d['opponent_state'], d['soccer_state'], 0 if i%MAX_FRAMES < n_trajectories/2 else 1),
                                d['ball_velocity'], d['kart_velocity'][1], d['opp_velocity'][0], d['opp_velocity'][1]]) for i, d in enumerate(train_data_imitation)]).to(device).float()
    ...
    save_model(action_net, 'state_agent.pt')
...
# Collect the data for dagger
    ...
    train_features_dagger0 = torch.stack([torch.cat([network_features(d['player_state'][0], d['opponent_state'], d['soccer_state'], 0 if (i % MAX_FRAMES) < n_trajectories/2 else 1),
                                                     d['ball_velocity'], d['kart_velocity'][0], d['opp_velocity'][0], d['opp_velocity'][1]]) for i, d in enumerate(train_data_dagger)]).to(device).float()
    print("train features dagger (kart 0) shape: ", train_features_dagger0.shape)

    print_val = train_data_dagger[0]['action0']

    train_features_dagger1 = torch.stack([torch.cat([network_features(d['player_state'][1], d['opponent_state'], d['soccer_state'], 0 if (i % MAX_FRAMES) < n_trajectories/2 else 1),
                                                     d['ball_velocity'], d['kart_velocity'][1], d['opp_velocity'][0], d['opp_velocity'][1]]) for i, d in enumerate(train_data_dagger)]).to(device).float()
